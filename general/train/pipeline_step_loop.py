    ...
    *,
    ...
    for step_idx in range(loop_steps):
        ...
        if should_eval and state["current_model_path"]:
            ...
            # when the latest candidate is clearly weaker than history-best,
            # continue selfplay from history-best instead of reinforcing a bad policy.
            if state["periodic_eval_summaries"]:
                latest_eval = state["periodic_eval_summaries"][-1]
                history_wr_raw = latest_eval.get("history_best_win_rate")
                history_wr = float(history_wr_raw) if isinstance(history_wr_raw, (int, float)) else None
                if history_wr is not None and history_wr < 0.5 and state["history_best_model_path"]:
                    history_best_path = str(state["history_best_model_path"])
                    history_best_model = Path(history_best_path)
                    if history_best_model.exists():
                        state["current_model_path"] = history_best_path
                        if best_checkpoint_path.exists():
                            state["current_checkpoint_path"] = str(best_checkpoint_path)
                        copy_model_pair(
                            src_model_path=state["current_model_path"],
                            src_checkpoint_path=state["current_checkpoint_path"],
                            dst_model_path=latest_model_path,
                        ...
                        print(
                            f"[train] rollback to history_best: step={step_idx + 1} "
                            f"history_best_win_rate={history_wr:.3f} "
                            f"model={state['current_model_path']}",
                            flush=True,
                        )
